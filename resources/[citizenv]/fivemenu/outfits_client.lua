...
AddEventHandler("vmenu:supdateChar", function(o, t, th, f, fi, s, se, e, n, te, el, tw, thi, fo, fif, si, y, u, sev, hat, hattext, glass, glasstext, watch, watchtext, beard, beardtext)
  model = GetHashKey(sev)
  RequestModel(model)
  Citizen.Trace("vmenu:supdateChar")
  while not HasModelLoaded(model) do -- Wait for model to load
    RequestModel(model)
    Citizen.Wait(0)
  end
  ...
  if GetEntityModel(GetPlayerPed(-1)) == 1885233650 then   
	...
	--SetPedPropIndex(GetPlayerPed(-1), 0, hat, hattext, 0)
	--SetPedPropIndex(GetPlayerPed(-1), 1, glass, glasstext, 0)
  --SetPedPropIndex(GetPlayerPed(-1), 6, watch, watchtext, 0)
  TriggerServerEvent("weaponshop:GiveWeapons")
  VMenu.updatedChar = true
end)
...
AddEventHandler("vmenu:updateCharInShop", function(args)
	Citizen.Trace("vmenu:updateCharInShop")
  model = GetHashKey(args[19])
  RequestModel(model)
  while not HasModelLoaded(model) do -- Wait for model to load
    RequestModel(model)
...
function menuOutfits(id, no, cat)
  VMenu.ResetMenu(8, "Tenues", "cloth")
  VMenu.AddMenu(8, "Tenues", "cloth") -- default = Header "Texte" sur fond bleu
  VMenu.AddFunc(8, "~r~Retour", "vmenu:OutfitsOG", {}, "Retour")
  VMenu.AddSep(8, cat)
  local i = 0
