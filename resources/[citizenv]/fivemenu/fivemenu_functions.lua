...
AddEventHandler("vmenu:getGarageGouv", function(target, vehicule) -- target = Dernier joueur à avoir parlé, pas besoin ici. Mais obligatoire !
  TriggerServerEvent('jobsmayor:vehGarage', vehicule)
end)

AddEventHandler("vmenu:getArmoryGouv", function(target, idGun) -- target = Dernier joueur à avoir parlé, pas besoin ici. Mais obligatoire !
  TriggerServerEvent('jobsmayor:wepArmory', idGun)
end)
AddEventHandler("vmenu:getBateauGarage", function(target, vehicule) -- target = Dernier joueur à avoir parlé, pas besoin ici. Mais obligatoire !
  TriggerServerEvent('jobspolice:vehBateauGarage', vehicule)
end)
AddEventHandler("vmenu:toGarageGouv", function(target) -- target = Dernier joueur à avoir parlé, pas besoin ici. Mais obligatoire !
  if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
    DrawNotif("~r~Sortez du véhicule !")
  else
    TriggerServerEvent('jobsmayor:vehtoGarage')
  end
end)
...
AddEventHandler("vmenu:getGouvHelicoGarage", function(target, vehicule) -- target = Dernier joueur à avoir parlé, pas besoin ici. Mais obligatoire !
  TriggerServerEvent('jobsmayor:vehHelicoGarage', vehicule)
end)
