...
AddEventHandler('hostingSession', function()
    ...
    if currentHosting then
        print('SESSION HOST LOCKING FOR ' .. source .. ' -> WAIT')
        ...
        table.insert(hostReleaseCallbacks, function()
            print('SESSION HOST LOCKING FOR ' .. source .. ' -> FREE')
    ...
    if GetHostId() then
        if GetPlayerLastMsg(GetHostId()) < 1000 then
            print('SESSION HOST LOCKING FOR ' .. source .. ' -> CONFLICT')
    ...
    print('SESSION HOST RESULT FOR ' .. source .. ' -> GO')
